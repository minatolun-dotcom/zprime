...
- **First boot with a new company (R-42 drill):** a fresh company starts with the standard account groups + seeded accounting ledgers (cash/bank/duties) but **no units and no godowns** — create those masters before your first inventoried item (Tally behaves the same). Typing an unknown ledger/item name in a voucher and pressing Enter offers inline quick-create (Tally Alt+C analogue); in the brief moment while a freshly opened voucher screen is still loading its pick lists, Enter can take that create path even for a name that exists — press Esc and re-pick, nothing is saved and no duplicate can be created.
