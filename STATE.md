...
**Last updated:** 2026-09-24 — **v1.51.0 RELEASED (R-54 TallyPrime shortcut parity + the v1.50.1 operator fixes) — commit `b31201115147dfe91481d303018d854ab4b1dd02`, annotated tag `e37e7b94d96fa8caded01841c5f32ee3ac2ef9b9`, pushed.** Alt+G = **Go To** command palette on every screen (type-to-filter over all Gateway destinations; Create/Alter deduped; Esc closes without history-back) — apply-GST moved to **Alt+J** (Tally's statutory-adjustment slot; r34/r35 re-anchored). Gateway binds every seeded voucher F-key data-driven (F4/F6/F7 from any screen); **Alt+D** delete / **Alt+X** cancel vouchers from the keyboard (confirm-guarded, R-02 semantics); **F3** = change company everywhere; **+/-** step the report period. v1.50.1 fixes folded in: master-page Esc-back, Gateway as the last stop (history barrier; Companies only via Switch Company/Logout; ← only with a trail), second-pass shortcut slots W/2/4/5/6/7/8/0/J/Z (Company Settings letterless — all letters+digits taken). Shared menu model: `client/src/lib/gatewayMenu.ts`. Study: `R-54_INVESTIGATION.md`; official map: `TALLYPRIME_SHORTCUTS_REFERENCE.md`. Verified: r54 **26/26**, r53 **43/43**, r34 20, r35 23, r20/r26/r27, run.js 153 fresh volume, final 948, smoke 39. Below: v1.50.0 **RELEASED** (R-52 + R-53 + R-53c) — commit `7d0dcf2`, tag `e09d084`, pushed. R-53 (revised on feedback): Gateway mirrors TallyPrime — company info rail left (name wraps, nothing overflows), general headings only at rest (Vouchers/Day Book/Create/Alter/Reports/Utilities; masters under Create+Alter), one heading's contents right, duplicate Shortcuts card removed, all 31 labels verbatim. R-53c (operator rules): hot letters globally unique — no two options share a shortcut letter (name-clash items letterless, arrows/Enter/click reachable; Create/Alter one shared option set, deduped by target); Esc = true history-back (daybook→voucher→Esc→daybook; gateway F5→voucher→Esc→gateway; modal Esc still claims first); Gateway rail F5/F8/F9 only, F2 = date/period (Day Book date input focus, inert on Gateway). `r53_ui.js` 32/32. R-52: One-line text fit + full-width body, CSS-only: `.cell-nowrap`/`.pill` tokens on all date/number/pill cells, fkey rail w-64, max-width cap dropped, Masters Del line-box fix. Verified: run.js 153/153 fresh volume; final 948/948; smoke 39/39; r03/r14/r20/r26/r27/r33/r34/r46_drill green. Previous: **v1.49.0 RELEASED (R-51 — PWA install support).**
