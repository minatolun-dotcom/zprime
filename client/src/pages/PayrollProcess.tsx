...
export default function PayrollProcess() {
  ...
  return (
    <Shell title="Process Payroll" breadcrumb={[{ label: "Gateway", to: `/company/${cid}` }, { label: "Process Payroll" }]}>
      ...
      {result && (
        <div className="mb-3 rounded border border-green-200 bg-green-50 text-green-800 text-[13px] px-3 py-2">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <Card className="p-4">
          <div className="text-[13px] font-semibold mb-2">Setup summary</div>
        ...
        <Card>
          <div className="px-3 py-2 border-b border-slate-100 text-[13px] font-semibold">Recent payslips</div>
          <table className="report-table">
            ...
            <tbody>
              ...
              {(history ?? []).length === 0 && <tr><td colSpan={3} className="text-center text-slate-400 py-3">Nothing processed yet</td></tr>}
            </tbody>
          </table>
        </Card>
      </div>
