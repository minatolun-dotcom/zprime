...
## v1.54.0 (PENDING — R-60 cross-FY report comparison, implemented + verified; not yet released)
...
**Verification:** typecheck server+client clean · build clean · new `r60_ui.js` **16/16** (April-2025 company: toggle on the big three only — no toggle on Day Book; P&L current vs prior card-header spans; BS Prev column + as-of title + Cash-in-Hand group value; TB Prev column with prior-window title + prev totals + CSV column present only when on; F12 toggles race-free; off-state has no prev column; zero page errors) · full estate green on a fresh volume: r59 11, r58 15, r57 21, r56 22, r54 26, r53 43, r34 20, r35 23, r20 12, r26 12, r27 15, run.js **153/153**, final_regression **948/948**, smoke **39/39** · `git diff --check` clean
