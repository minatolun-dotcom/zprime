...
## v1.54.0 (PENDING — R-60 cross-FY report comparison + R-61 Tally-sectioned Gateway; implemented + verified; not yet released)

**R-61 reorganizes the Gateway to mirror TallyPrime (operator request: "look at how TallyPrime organizes its options in the Gateway and inside the Gateway — items, vouchers, etc — and try to match it. Now it's complicated and not seamless and convenient.")** — Option C of `R-61_INVESTIGATION.md` (operator-approved). TallyHelp's own description — "Gateway of Tally is organised into **Masters, Vouchers, Utilities, and Reports**" — becomes zprime's level 1: the middle column carries the four **section headers** (Masters / Transactions / Utilities / Reports) with items **directly visible** — Tally's seamlessness: Utilities' XML Import (X), Cheque Printing (J), Audit Trail (Z) and the seven headline reports (Balance Sheet B, Profit & Loss F, Trial Balance T, Cash/Bank H, Sales Register S, Purchase Register P, Stock Summary M) need **no drill to see or fire**; the nine-report tail (Receivables, Payables, GSTR-1/3B/9, TDS, TCS, Salary Register, Cheque Register) folds out in place behind a letterless **"Display More Reports"** expander (Tally's own pattern, `…` chip — punctuation, not pressable, same class as Company Settings' `·`). Panes survive exactly where Tally has them: V (Voucher types + payroll) and C/A (Create/Alter masters — zprime's masters are pages, not modes). **Every pre-R-61 hot letter keeps firing the same target** (muscle memory intact; R-then-T is now one T); the freed heading letters **R and U fire nothing** — retired, never reassigned, so R-53c's uniqueness arithmetic is untouched. The Go To palette (Alt+G) follows the new sections via `flattenMenu` (sections = the Tally area). Scope: `client/src/lib/gatewayMenu.ts` (sections via a `header` field + Display More Reports entry), `client/src/pages/Gateway.tsx` (section rendering + expander, `gateway-section-header`/`gateway-more-toggle`/`gateway-more-expanded` testids); zero server/schema/accounting change. Honest re-anchoring, nothing weakened: r53 43→**40** (the three removed checks were the R-pane assertions the reorganization retires — R opens Reports contents, B-from-inside-R, R-then-T; their substance lives on as direct-fire B and T checks), r20/r26/r27/r59 navigations updated to the sectioned layout (r59's U-pane check became the level-1 Utilities check it was asserting the spirit of).
...
**Verification:** typecheck server+client clean · build clean · new `r61_ui.js` **43/43** (four section headers in order; Utilities + headline reports at level 1 with no keypress; tail hidden until expanded, then 9 items with letters unchanged and keyboard 5 firing Receivables from it; Esc collapses; freed R/U fire nothing; the full 12-letter muscle-memory loop X/J/Z/B/F/T/H/S/P/M/K(+W-in-V) fires the same targets; V/C/A panes open with their letters; promoted B fires from inside the Create pane; palette serves XML Import under Utilities and GSTR-9 under Reports; Settings one click; zero page errors) · re-anchored suites on the same stack: r53 **40/40**, r20 12, r26 12, r27 15, r59 11 · full estate green on a fresh volume: r61 43, r60 16, r59 11, r58 15, r57 21, r56 22, r54 26, r53 40, r34 20, r35 23, r20 12, r26 12, r27 15, run.js **153/153**, final_regression **948/948**, smoke **39/39** · `git diff --check` clean
