...
## v1.63.0 — R-73: TallyPrime voucher feature batch — optional drafts, duplicate, zero-value opt-in, per-line narration, item discount, Sale/Purchase Orders + Order Book, bank txn types + BRS (RELEASED 2026-09-26 — commit `5b4972d74b08be864f4f3d180f9589c0c313f51c`, annotated tag `da4de817df1a8a307c2c44a9e703a3268dcdc06f`; push via secrets token)

**R-73 (operator-approved findings F-73-1…7 from `R-73_INVESTIGATION.md`; migration 0018, additive).** **Optional drafts:** `vouchers.is_optional` — Ctrl+L parks a half-entered voucher; drafts are excluded from every accounting surface (all 24 sites filter `isOptional=false` alongside `isCancelled`), carry provisional `OPT-n` numbers (new partial unique index; posted-class index gains `WHERE is_optional = false`) without consuming the serial counter; Day Book draft lens (default Hide drafts / Optional only / Include drafts) + Accept action; `POST /vouchers/:id/accept` stamps the real number and re-runs the guards; drafts refuse cancel, delete freely. **Duplicate:** Ctrl+D into a fresh form (Alt+2 is zprime's Alt-digit alias). **Zero-value opt-in:** per-type `allow_zero_value_entries` (Tally F12) permits zero lines/totals on that type only. **Per-line narration:** `voucher_entries.narration` + grid column. **Item discount:** `inventory_entries.discount_pct` — amounts book NET (GST on net); stock values at net (10 × 100 − 10% = 900, suite-proven). **Orders:** Sale/Purchase Order types (migration backfills every existing company; keyless — a/so slots taken): commitment-only (ledger entries refused; `kind="order"` stock rows skipped by valuation and the availability guard; no bills/outstanding); Against-Order picker on invoices; **Order Book** report (ordered/fulfilled/pending). **Banking:** `bank_txn_type` (cheque|rtgs|neft|upi|other) + Cheque Register column; `reconciled_at` + Bank Reconciliation endpoint/view/PATCH — operator-confirmed legs only, statement balance = book closing − un-cleared. Full estate green on a fresh volume (run 153, smoke 39, final 952, batteries 88/65/61/29, all r-suites baseline, r74 33/33 new, personas 23/26/25); typecheck + build clean (427.68 kB / 120.77 kB gzip).

## v1.62.0 — R-72: master editor save chords — Ctrl+A accepts (Tally's universal Accept), Ctrl+S claimed as its alias (RELEASED 2026-09-26 — commit `987ec61c83d62e424ea6441fb34c558b5ed80cce`, annotated tag `76e5f098011c3c1223ac80c08346e62dee0638c1`; push via secrets token)

**R-72 (client-only, operator report: "alter Ctrl+S is not working as it tries to save the page").** MasterPage's Alter button advertised a Ctrl+S shortcut no key ever fired — the browser's Save dialog swallowed the chord. Tally parity: Ctrl+A IS Tally's universal Accept and already zprime's voucher save key, so the master editor (Create/Alter slide-over) now saves with **Ctrl+A — scoped to the open editor only** (native select-all untouched everywhere else) — and **Ctrl+S is claimed as an alias** so the Save dialog can never interrupt master editing. Both submit via `requestSubmit()` through the form (native required-field validation still runs — a missing field keeps the editor open); the button hint is corrected to "Alter (Ctrl+A)". New `r72_ui.js` **11/11** (chord saves on Create and Alter, no dialog, select-all intact outside, validation + duplicate-409 surfacing, Esc contract, zero page errors). Full estate green on a fresh volume (run.js 153, smoke 39, final 952, attack 88, fix-regression 65, reconcile 61, attack2 29, all r-suites baseline, r72 11, personas 23/26/25); typecheck server+client clean; build clean (415.47 kB / 117.97 kB gzip).

## v1.61.0 — R-71: overdue bills highlight red on the Outstanding views — direction-aware, print-safe day marker (RELEASED 2026-09-26 — commit `6562dcf0167d03f83998d2cfcff0edde0a9407a8`, annotated tag `50e71e92463e4ae1e3deb2b5ce04d1a432294b16`; push via secrets token)

**R-71 (client-only, operator request following R-70).** R-70 made every bill row visible on the face, but a bill past its due date looked identical to a current one. `OutstandingView` (Bills Receivable / Bills Payable) now flags **overdue** bills: it gains a `debtSign: 1 | -1` prop (receivables=1, payables=-1) and a bill is overdue only when it has a dueDate AND is open in the debt direction (receivables `amount > 0.004` with debtSign=1; payables `amount < -0.004` with debtSign=-1 — an advance never flags red) AND `dueDate < asOf` (the report's own as-of, so as-of time travel stays honest). Overdue rows get a `bg-red-50` tint + red-700 bill name/amount/due text, a print-safe color-independent `· overdue Nd` day-count marker in the Due cell, and a red "overdue" chip (`print-keep`) on the party header when any of its bills is overdue. New `r71_ui.js` **15/15** (past-due flags; future-due/no-due clean; advance with past dueDate does NOT flag — direction-aware; party chip; payables mirrored; print-media marker; zero page errors). Full estate green on a fresh volume (run.js 153, smoke 39, final 952, attack 88, fix-regression 65, reconcile 61, attack2 29, all r-suites baseline, r71 15, personas 23/26/25); typecheck server+client clean; build clean (415.31 kB / 117.90 kB gzip); `git diff --check` clean.

## v1.60.0 — R-70: Outstanding views render bill rows per bill — bill-name labels visible by default (Tally bill-wise display) (RELEASED 2026-09-26 — commit `687b61b30abf887054934f978c8f48474d3e13c1`, annotated tag `8db1f4faf8c6269ec2edfa2d2211d7c68fe3f610`; push via secrets token)

**R-70 (client-only).** R-69 put every party's bills (On Account / Opening Balance / named bills) in the API, but the Outstanding UI hid them behind a per-party click — bill labels were invisible until drilled and never printed. `OutstandingView` (Bills Receivable / Bills Payable) now expands each party card's bill table **by default** (collapse still available): every bill row renders with name, date, amount and due date; synthetic bills render italic-muted vs named bills; rows are the report body and stay visible in print media; CSV per-bill contract unchanged. New `r70_ui.js` **19/19** (default-open rows for all three bill kinds, amounts + due date, collapse/expand, print visibility, CSV one-row-per-bill with bill names). Full estate green on a fresh volume (run.js 153, smoke 39, final 952, attack 88, fix-regression 65, reconcile 61, attack2 29, all r-suites baseline, r70 19, personas 23/26/25); typecheck server+client clean; build clean (414.53 kB / 117.66 kB gzip); `git diff --check` clean.

## v1.59.0 — post-release audit fixes (login 401 feedback, `vouchers?type=` hardening, three persona suites) + R-69 On Account outstanding for non-bill-wise parties (RELEASED 2026-09-26 — commits `6dbddd8` (audit) + `765948d3267bf9ce00d299655c93a8e01f47d8f6` (R-69), annotated tag `16b2c0cebc67e04a6e23a2da735fd34776f6d608`; push via secrets token)

**Three-persona audit of v1.58.0 (beginner / professional accountant / hostile user; ~1,460 checks incl. the full existing battery), then the fixes its findings earned.** **(1) Login 401 feedback (beginner persona):** the shared `api()` helper redirected on every 401, so a wrong password reloaded /login, blanked the form, and showed nothing; the login endpoint is now exempt from the global redirect and the form renders the server's message. **(2) Hardening (hacker persona):** `GET /vouchers?type=<non-numeric>` reached SQL with NaN and 500'd — now a clean 400. **(3) R-69 (Tally parity, closing the audit's receivables finding):** party ledgers without bill-wise tracking never produced `bill_allocations` rows, so their outstanding was invisible in Bills Receivable/Payable; `billWiseOutstanding` computes each party ledger's net from entries and merges the residual not covered by entry-derived bills as an **On Account** bill, excluding "opening" bills from the covered base so the R-07 opening merge never double-counts; bill-wise parties keep their named bills exactly (no phantom merge — allocation-covered balances produce zero residual). New `r69_ui.js` **17/17**; beginner persona re-anchored to assert the On Account bill; final_regression's Cust A expectation honestly re-anchored (5660 → 13660: its 8000 of P&L fixtures had no allocations and were invisible pre-R-69). Estate green on a fresh volume: run.js 153, smoke 39, final 952, attack 88, fix-regression 65, reconcile 61, attack2 29, all 19 r-suites baseline, r69 17, personas 23/26/25; post-abuse DB integrity clean (Σ signed amounts 0.00).

## v1.58.0 — R-68 IRP SignedQRCode/SignedInvoice storage (migration 0017) + IRN QR on the invoice face (RELEASED 2026-09-26 — commit `39ffa28073dcfa6a6e18ebb21541bbc71b36a6bf`, annotated tag `faebcb1abc0f3539f28b93de71d8d804d3624af8`; push via secrets token)

**R-68 (approved from the R-67 investigation's top compliance finding, Option D; investigation commit `37b529d`, `R-68_INVESTIGATION.md`).** The e-invoice submission ledger discarded NIC's `SignedQRCode`/`SignedInvoice` from the GENIRN response, so printed e-invoices could not carry the legally expected IRN QR. Implemented: **(1) Migration 0017** — additive-only, generated via the new `server/scripts/gen-0017.ts` (drizzle-kit programmatic API, the gen-0013 pattern; journal 17→18, verified applied on a fresh volume): `irp_submissions.signed_qr_code` + `signed_invoice` nullable text columns. **(2) Server** — `submitEInvoice`'s GENIRN accept branch persists `resp.SignedQRCode`/`resp.SignedInvoice` (rejected rows stay NULL); the verbatim `response` jsonb contract is unchanged and every EWB path is untouched; `maskSubmission()` (documented passthrough) keeps the stored row safe to return. **(3) Mock fidelity** — `scripts/mock_irp.js` genirn now emits deterministic `SignedQRCode` (base64 of `mock-signed-qr:<irn>`) + `SignedInvoice`. **(4) Client** — `qrcode.react@^4.2.0` (zero runtime deps, SVG output); `InvoicePrint.tsx` owns the submissions query (queryKey `["einvoice-submissions", cid, voucherId]`, staleTime 60s) and renders `IrnQrStrip` (`data-testid="irn-qr-strip"`: QRCodeSVG size 84 + IRN + Ack No/date) on the invoice face only when an accepted e-invoice with a `signedQrCode` exists — VoucherScreen untouched, zero accounting surface. New `r68_ui.js` **17/17** (mock accept → signed artifacts on the submission → QR strip on the Sales alter face: IRN text, Ack No, SVG present, visible in print media via emulateMedia; honest no-IRN negative on a second sale; zero page errors); `r46_drill.js` +3 → **42/42** (signed QR+invoice on the accepted row; history exposes signedQrCode; duplicate 409 re-surfaces the stored QR); `final_regression.py` +4 → **952/952** (SignedQRCode/SignedInvoice on the submission payload; DB columns persisted; rejected submission carries NULL signed QR). Full estate green on a fresh volume, every suite exactly once (r53 43, r54 26, r56 22, r57 21, r58 15, r59 12, r60 16, r62 15, r63 25, r64 13, r65 12, r66 75, r34 20, r35 23, r20 12, r26 12, r27 15, run.js 153, smoke 39); typecheck server+client clean; build clean (bundle 414.27 kB / 117.55 kB gzip from qrcode.react); `git diff --check` clean.

## v1.57.0 — R-66 export/printing: CSV + Print on every report view, Day Book + Audit Trail export, invoice printing (RELEASED 2026-09-26 — commit `a7cb6eebca0f7a5bd337d0dd93971eb217567177`, annotated tag `bc657a26e8d97a1c857521e457c23329e05b7bb3`; push via secrets token)

**R-66 Option C (operator-approved; investigation commit `0288e2f`, `R-66_INVESTIGATION.md`).** The investigation found the README's "Export CSV" claim covered only 2 of 17 report views and ChequePrint was the sole print surface — no PDF/XLSX/server export anywhere. Option C closes the everyday gaps: **(1) CSV everywhere** — a shared `ReportActions` bar (Export CSV + Print, `print:hidden`) with a `PrintHead` provenance header now renders on all 19 report views; `csvDownload` gained optional meta rows (company/GSTIN + report title/period) above the table; tree views (Balance Sheet, Chart of Accounts) flatten with an explicit **Level column + 2-space-per-level indented names** (operator-approved form); TB appends a Totals row and keeps Prev Closing when compare is on; P&L exports a unified Dr/Cr statement; every other view (Ledger Vouchers, Group Summary, Cash/Bank, Registers, Stock Summary both branches, Outstanding flat party/bill rows, GSTR-1 B2B, GSTR-3B sections+net, GSTR-9 flattened duty tables, TDS, TCS, Salary Register, Cheque Register) exports its displayed data; **Day Book** and **Audit Trail** gained export buttons (`daybook-export-csv` / `audit-export-csv`) with meta rows. **(2) Print stylesheet** — `@media print` block hides app chrome (header, fkey rails `print:hidden`; buttons blanket-hidden with a `print-keep` escape), de-shadows cards with break-inside-avoid, and repeats `report-table` theads per page. **(3) Invoice printing** — new `client/src/components/InvoicePrint.tsx`: a Tally-style invoice face for **Sales/Delivery Note** vouchers in edit mode — company header (mailing name/address/city/pincode/GSTIN/phone), doc title, no/date/ref, Billed-to block from the party master (fetched live; refreshed after save via re-GET), place of supply, inventory table (item/HSN/qty/rate/amount), accounting lines with duty rows indented, taxable+GST summary, **amount-in-words** via the extracted shared `amountWords()` (ChequePrint now imports it), narration + signatory; a "Print Invoice" button (`print-invoice`) sits next to Cancel; rendered as a print-only fixed layer + on-screen preview card. **En-route defect found by the suite:** the client fetched cheque-register under `/api/c/:cid/reports/cheque-register` while the server mounts banking at `/api/c/:cid/cheque-register` — the Cheque Register view never rendered; URL memo fixed. New `r66_ui.js` **75/75**; full estate green on a fresh volume (run.js 153, final 948, smoke 39, all 16 r-suites green). Client-only; zero server/schema/accounting change.

## v1.56.0 — R-65 Gateway voucher-hint restore + Day Book data-driven voucher map/rail (RELEASED 2026-09-25 — commit `e7f5f06676f5f7ad88c73c5121953a7eabff7d3f`, annotated tag `6c89a8b744e2cedef8d6439787120c9fa4a077de`; pushed 2026-09-25 — remote verified == local)
