...
**Last updated:** 2026-09-18 — **v1.25.0 RELEASED** (R-26 GSTR-9 annual return). Phase IDLE; next session asks for R-27 direction.
...
- **Current phase:** `IDLE` — v1.25.0 released; the next R-item requires its own investigation → review → approval cycle — see `DEVELOPMENT_PROTOCOL.md`
- **Current task:** none. Last: R-26 (GSTR-9 annual return — `services/gstr9.ts` tables 4/5/6-7/8/9/12 over one FY window, Table-8 duty-ledger ITC reconciliation with honest difference semantics, Table-9-vs-3B consistency cross-check, Gstr9View + Gateway entry; no migration). SAME commit carried the release-process amendment: RELEASED protocol now includes `git push origin main --tags` (backlog push v1.0.0–v1.24.0 PENDING — no session credentials). Investigation: `R-26_INVESTIGATION.md`.
...
- **Next permitted action:** on instruction — release gate on the final tree, commit `Release v1.25.0: GSTR-9 annual return`, annotated tag `v1.25.0`, integrity + immutability verification, then attempt the remote push (expected BLOCKED until credentials exist).
...
- **⚠ PUSH BACKLOG (standing release step, adopted R-26 era):** the remote `origin/main` is at the pre-v1.3.0 import commit; ALL 26 tags and 27 commits exist only locally. Push `git push origin main --tags` is **BLOCKED** in-session (no GitHub credentials available) — supply a PAT / set up SSH / push manually. RELEASES.md ledger rule 4 records this; DEVELOPMENT_PROTOCOL.md RELEASED state now includes the push step for all future releases.
